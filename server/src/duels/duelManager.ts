    ...
    try {
      ...
      if (passedAll) {
        ...
        if (room.winnerId === null) {
          // This player is the first winner
          room.winnerId = userId;
          room.status = 'completed';
          this.rooms.set(roomId, room);
          io.to(roomId).emit('duelEnded', { winnerId: userId, room }); // Broadcast winner
          console.log(`Duel room ${roomId} completed. Winner: ${player.username}`);
        } else {
          // Another player already won
          socket.emit('submissionResult', { success: true, message: 'You passed all tests, but someone else won first.', results });
        }
      } else {
